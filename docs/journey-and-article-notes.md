...
- **The codec asset seam.** Squoosh resolves codec `.wasm`/worker URLs in ways a
  Vite/SvelteKit static build doesn't do for free. Solution: a data-driven asset
  manifest generated by `scripts/sync-sveltekit-app.mjs` (logical keys → physical
  assets → precache), plus a wrapper-patch step so Vite doesn't emit duplicate
  WASM. (This generator is also the backbone of the codec + threading work in
  Article 2 — a nice through-line.)
- **One pipeline for single-image and bulk.** Both go through
  `imagePipeline.compressImage` via a generated worker-bridge, so there's no

...
Working tree at last update: progress dashboard reflects the Node 24 build baseline, browser smoke coverage for saved-settings import, real resize processing to a `64x64` WebP blob, service-worker-disabled app-shell loading, service-worker controller guards, refreshed browser support policy, static-build utility cleanup, shared filename sanitization, shared abort-error checks, local folder rename to Sqush, a wider direct-open HTML dashboard at `docs/progress-dashboard.html`, `npm run dashboard` for change-triggered dashboard reloads with graceful already-running detection, browser smoke preview-port retry handling, app-shell listener and async cleanup, editor async undo/error cleanup, worker bridge disposal cleanup, expander transition cleanup, custom element observer/listener cleanup, file-drop handler type cleanup, codec package/build metadata, codec source reference docs, and expanded single-image pure helpers. Bulk helpers include a framework-neutral barrel export, shared import result recording, shared active status checks, shared canonical status validation, shared current-output/export readiness predicates, shared queued reset behavior, import-to-session helper, append-import helper, structured import rejection reasons and summary counts, shared SVG-aware source decode path, process-plan helper, selected-job detail selector, queue-state selector, shared queue counter deltas across direct and batch transitions, export-plan completion helper, drift-tolerant session counters including add-job counter normalization, strip item selectors, queue-aware session summary selectors, and metadata-only snapshot restore from parsed or serialized snapshots for future UI/Svelte consumption. Initial app entry/share-target/open/render state and single-image work-start abort/controller cycling, source decode/preprocess execution, source workflow orchestration, side workflow orchestration, update workflow coordination, update queue runtime scheduling, editor update workflow orchestration, editor cleanup runtime orchestration, side job state patches, side settings state patches, side copy/import state patches, side-copy workflow, initial editor state, source lifecycle state, source decode success state, viewport state, editor work planning, work-start scheduling, editor update scheduling options, image update scheduling, active job bookkeeping, active job completion bookkeeping, work abort planning, side job run selection, runnable side job records, side job execution, runnable side-job loop orchestration, side job execution planning, side encode decisions, side result/cache assembly, planned current/latest image job-state derivation, display settings, result labels, image-contain decisions, render display selectors, composed render state, compression render state, option panel render state, option async load/listener workflow, option initial/load state, panel layout state, output draw state, output render state, output update plan, output mount/update workflow, output control state, output initial control state, output control interaction/event-retarget/focus state, output preview state, result size display state, result initial loading state, result loading timer workflow, result loading effects and visibility cleanup, result download state, processor control parsing, processor enabled transitions, processor option merges, resize preset state, range input and range display state, default source resize side updates, orientation resize side updates, preprocessor-change state updates, side loading/result updates, side undo restoration, saved-settings key/label/payload construction, saved-settings read/write actions, saved-settings save/import feedback actions and workflows, saved-settings availability and availability updates, side-copy URL behavior and feedback actions, processing error messages, encoder select state, processor control state, side reset URL cleanup, side settings mutations, initial/default/saved side-state updates, expander state transitions, unmount URL cleanup, document-title/loading selectors, new-file lifecycle handling, editor update effects, supported encoder filtering, and the Svelte migration handoff are partly extracted or modernized. Stateless option controls no longer use empty-state Preact classes. Maintained source no longer uses `componentWillReceiveProps`. Browser smoke now covers real saved-settings import. Project identity is documented so the old archive name is not confused with current Sqush branding. Run `git status --short --branch` for the exact state.

Latest recent committed work at last update:

- Current local work: output render state helper
- Current local work: compression render state helper
- Current local commit: Extract update workflow

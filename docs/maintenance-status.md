...
Working tree at last update: progress dashboard reflects the Node 24 build baseline, browser smoke coverage for saved-settings import, real resize processing to a `64x64` WebP blob, service-worker-disabled app-shell loading, service-worker controller guards, refreshed browser support policy, static-build utility cleanup, shared filename sanitization, shared abort-error checks, local folder rename to Sqush, a wider direct-open HTML dashboard at `docs/progress-dashboard.html`, `npm run dashboard` for change-triggered dashboard reloads with graceful already-running detection, browser smoke preview-port retry handling, and expanded single-image pure helpers. Bulk helpers include a framework-neutral barrel export, shared import result recording, shared active status checks, shared canonical status validation, shared current-output/export readiness predicates, shared queued reset behavior, import-to-session helper, append-import helper, structured import rejection reasons and summary counts, shared SVG-aware source decode path, process-plan helper, selected-job detail selector, queue-state selector, shared queue counter deltas across direct and batch transitions, export-plan completion helper, drift-tolerant session counters including add-job counter normalization, strip item selectors, queue-aware session summary selectors, and metadata-only snapshot restore from parsed or serialized snapshots for future UI/Svelte consumption. Initial app entry/share-target/render state and single-image side job state patches, side settings state patches, side copy/import state patches, initial editor state, source lifecycle state, source decode success state, viewport state, editor work planning, work-start scheduling, editor update scheduling options, image update scheduling, active job bookkeeping, active job completion bookkeeping, work abort planning, side job run selection, side job execution planning, side encode decisions, side result/cache assembly, planned current/latest image job-state derivation, display settings, result labels, image-contain decisions, render display selectors, composed render state, option panel render state, panel layout state, output draw state, output control state, output control interaction/event-retarget state, output preview state, result size display state, result loading effects, result download state, processor control parsing, processor enabled transitions, processor option merges, resize preset state, default source resize side updates, orientation resize side updates, preprocessor-change state updates, side loading/result updates, side undo restoration, saved-settings key/label/payload construction, saved-settings read/write actions, saved-settings save/import feedback actions, saved-settings availability and availability updates, side-copy URL behavior and feedback actions, processing error messages, encoder select state, processor control state, side reset URL cleanup, side settings mutations, initial/default/saved side-state updates, expander state transitions, unmount URL cleanup, document-title/loading selectors, new-file lifecycle handling, editor update effects, and supported encoder filtering are partly extracted or modernized. Stateless option controls no longer use empty-state Preact classes. Maintained source no longer uses `componentWillReceiveProps`. Browser smoke now covers real saved-settings import. Project identity is documented so the old archive name is not confused with current Sqush branding. Run `git status --short --branch` for the exact state.

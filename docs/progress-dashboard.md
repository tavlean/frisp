...
| Bulk backend foundation              |      98% | Framework-neutral helpers now cover import, shared import result recording, structured import rejection reasons, import-to-session creation, append-import workflows, injected MIME sniffing, settings lookup, process-plan creation, safe settings-change workflows, queueing, queue-state selectors, shared active status checks, shared current-output/export readiness predicates, shared queued reset behavior, full queue draining, active/incomplete requeueing, active-job cancellation, shared queue counter deltas across direct and batch queue transitions, derived session counters, drift-tolerant session mutations, drift-tolerant queue scheduling and transitions, metadata-only session snapshots with validated parsing/restoration, per-job size state, selected export plans, export-plan completion, export summaries, stale export guards, selected-job detail, strip and queue-aware session summary view models, command action state, processing, and URLs. |
| Repo simplification                  |      57% | Dependency/script cleanup, Node 24 baseline alignment, storage centralization, storage failure handling, result-cache coverage, shared abort handling, shared/service-worker type cleanup, static-build utility cleanup, tested single-image work planning/work-start scheduling/side encode planning/job derivation/display/source/preprocessor/saved-settings/saved-settings-availability/saved-settings-actions/initial-side-state/side-state/editor-update/encoder-support helpers, removal of the main editor's deprecated prop-change lifecycle, and removal of maintained app/lib TS suppressions have reduced some legacy loose typing, scattered browser APIs, and old Preact assumptions. Codec surface, Preact coupling, and custom Rollup complexity still remain.                                                                                                                                                                                                         |
| Svelte/SvelteKit migration readiness |      42% | Shared pipeline, single-image work planning/work-start scheduling/side encode decisions/job derivation/source defaults/preprocessor changes/display render selectors/editor update effects/encoder support/saved-settings availability/saved-settings-actions/initial-side-state/side-state helpers, removal of the main editor's deprecated prop-change lifecycle, and bulk logic extraction have started, and the current Svelte 5/SvelteKit migration guidance is documented. UI migration should wait until behavior is better tested.                                                                                                                                                                                                                                                                                                                                                                                                                                             |

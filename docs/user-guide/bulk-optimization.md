...
Bulk mode keeps the same local-first promise as the single-image editor. Sqush
...
- Drag several image files onto Sqush.
- Click the big select button in the middle and pick more than one image.
- Click **Choose folder** to import a folder.

If you import **2 or more supported images**, Sqush opens the batch view. If you
import **1 image**, Sqush opens the normal single-image editor exactly as before.
...
image's optimized version would be bigger than the original, Sqush puts the
...
Use the remove button on an image to take it out of the batch. Sqush shows a
...
- **Keep originals when larger protects the ZIP, not the preview.** You may
  ...
  into the ZIP.
- **Folder import is for reading, not saving back.** Sqush downloads a ZIP; it

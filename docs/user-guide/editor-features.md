...
- **Compare as…:** Click **Compare as…** to turn the left panel into a second encoder panel. Pick WebP, AVIF, JPEG XL, JPEG, or PNG from the menu and Presk encodes the left side too, so you can compare two compressed formats live against each other instead of only Original vs. WebP.
...
- **Undo/Redo is per-image and instant.** Stepping back never re-compresses — it replays a result Presk already has. But loading or replacing the image clears the history, so undo can't reach across to a previous image (a replace itself can't be undone).

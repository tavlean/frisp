...
> The settings below are **community recommendations** drawn from web.dev's AVIF guide and encoder-community practice, translated into Frisp's UI terms (its Quality slider is 0–99 and its Effort slider is 0–10, where higher Effort = slower/smaller). They are advice, not new defaults; the factual ranges and defaults above are unchanged. Sources are listed at the end.

...
export default function () {
   ...
   */
  async function generateWorkerFile(workerImports, outputFileName) {
    ...
    await fsp.writeFile(join(workerBasePath, outputFileName), workerFile);
  }
  ...
  async function generateWorkerBridge(workerImports) {
    ...
    const activeWorkerBridgeTsNames = featuresWorkerBridgeTsNames.filter(
      ([_, name]) => !blockedWorkerMethodReasons.has(name),
    );
    ...
    const activeBridgeMeta = [
      autoGenComment,
      `import { activeMethodNames } from './surface';`,
      activeWorkerBridgeTsNames.map(
        ([path, name]) => `import type ${name} from '${path}';`,
      ),
      `export const methodNames = activeMethodNames;`,
      `export interface BridgeMethods {`,
      activeWorkerBridgeTsNames.map(([_, name]) => [
        `  ${name}(`,
        `    signal: AbortSignal,`,
        `    ...args: Parameters<typeof ${name}>`,
        `  ): Promise<ReturnType<typeof ${name}>>;`,
    ...
    await Promise.all([
      ...
      fsp.writeFile(
        join(workerBridgeBasePath, 'active-meta.ts'),
        activeBridgeMeta,
      ),
  ...
  async function generateWorkerFiles() {
    ...
    const activeWorkerImports = workerImports.filter(
      (workerImport) => !blockedWorkerMethodReasons.has(basename(workerImport)),
    );

    await Promise.all([
      generateWorkerFile(workerImports, 'index.ts'),
      generateWorkerFile(activeWorkerImports, 'active.ts'),
  ...
  async function generateFeatureMeta() {
    ...
    const sharedFeatureMeta = [
      ...
      processorMetaTsNames.map(
        ([_, name]) => `  ${name}: { meta: ${name}ProcessorMeta },`,
      ),
      `};`,
      `export type ProcessorType = keyof typeof processorMap;`,
      `export const preprocessorMap = {`,
      preprocessorMetaTsNames.map(
        ([_, name]) => `  ${name}: { meta: ${name}PreprocessorMeta },`,
      ),
      `};`,
      `export type PreprocessorType = keyof typeof preprocessorMap;`,
    ...
    const featureMeta = [
      ...
      `  preprocessorMap as preprocessorMetaMap,`,
      `  processorMap as processorMetaMap,`,
      `} from './shared';`,
      `export const encoderMap = {`,
      encoderMetaTsNames.map(
        ([_, name]) =>
    ...
    const processorFeatureMeta = [
      autoGenComment,
      `export type { ProcessorOptions, ProcessorState, ProcessorType } from './shared';`,
      `export {`,
      `  defaultProcessorState,`,
      `  processorMap,`,
      `  processorMap as processorMetaMap,`,
    ...
    const preprocessorFeatureMeta = [
      autoGenComment,
      `export type { PreprocessorState, PreprocessorType } from './shared';`,
      `export {`,
      `  defaultPreprocessorState,`,
      `  preprocessorMap,`,
      `  preprocessorMap as preprocessorMetaMap,`,
    ...
    await Promise.all([
      ...
      fsp.writeFile(
        join(featureMetaBasePath, 'processors.ts'),
        `${processorFeatureMeta}\n`,
      ),
      fsp.writeFile(
        join(featureMetaBasePath, 'preprocessors.ts'),
        `${preprocessorFeatureMeta}\n`,
      ),
    ]);
  }

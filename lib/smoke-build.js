...
function assertSharedImagePipelineSeam() {
  ...
  const bulkProcessorPath = path.join(
    process.cwd(),
  ...
  const compress = fs.existsSync(compressPath)
    ? fs.readFileSync(compressPath, 'utf8')
    : '';
  const bulkProcessor = fs.existsSync(bulkProcessorPath)
    ? fs.readFileSync(bulkProcessorPath, 'utf8')
    : '';
  assert(
  ...
  assert(
    pipeline.includes('export const imagePipeline = {') &&
      pipeline.includes('decodeSourceImage') &&
      pipeline.includes('preprocessImage') &&
      pipeline.includes('processImage') &&
      pipeline.includes('compressImage') &&
      pipeline.includes('decodeImage'),
    'Production image pipeline should expose the injected imagePipeline helper bundle',
  );
  assert(
  ...
  assert(
    compress.includes('pipeline: imagePipeline'),
    'Production Compress shell should consume the shared imagePipeline bundle',
  );
  assert(
    bulkProcessor.includes(
      'const defaultPipeline: BulkProcessorPipeline = imagePipeline',
    ),
    'Bulk processor default should consume the shared imagePipeline bundle',
  );
  assert(
...
function assertActiveServiceWorkerCacheSeam() {
  const activeToCachePath = path.join(
    process.cwd(),
    'src/sw/active-to-cache.ts',
  );
  const productionToCachePath = path.join(process.cwd(), 'src/sw/to-cache.ts');
  if (
    !fs.existsSync(activeToCachePath) ||
    !fs.existsSync(productionToCachePath)
  ) {
    return;
  }

  const activeToCache = fs.readFileSync(activeToCachePath, 'utf8');
  const productionToCache = fs.readFileSync(productionToCachePath, 'utf8');
  assert(
    activeToCache.includes("from './processor-support'") &&
      productionToCache.includes("from './processor-support'"),
    'Service-worker cache boundaries should share processor support detection',
  );
  assert(
    activeToCache.includes('entry-data:../features-worker/active'),
    'Active service-worker cache seam should use the active features-worker entry',
  );
  assert(
    activeToCache.includes('buildActiveAdditionalProcessorCacheUrls'),
    'Active service-worker cache seam should use active cache planning',
  );
  assert(
    !activeToCache.includes('codecs/wp2/'),
    'Active service-worker cache seam should exclude WebP 2 codec imports',
  );
  assert(
    productionToCache.includes('codecs/wp2/'),
    'Production service-worker cache should preserve current WebP 2 imports',
  );
}

function assertGeneratedFeatureMetadataSeams() {
  const generatedFeatureMetaFiles = [
    'src/client/lazy-app/feature-meta/processors.ts',
    'src/client/lazy-app/feature-meta/preprocessors.ts',
  ];

  for (const file of generatedFeatureMetaFiles) {
    const filePath = path.join(process.cwd(), file);
    if (!fs.existsSync(filePath)) continue;
    const source = fs.readFileSync(filePath, 'utf8');
    assert(
      !source.includes('/client') && !source.includes('client/runtime'),
      `Generated framework-neutral metadata seam should not import client entries: ${file}`,
    );
  }
}
...
assertActiveServiceWorkerCacheSeam();
assertGeneratedFeatureMetadataSeams();

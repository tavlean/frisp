...
function generateWebpWorkerEntry() {
  return [
    ...
    "import avifEncoder from 'sqush-generated/codecs/avif/enc/avif_enc';",
    ...
    "import webpDecoder from 'sqush-generated/codecs/webp/dec/webp_dec';",
    ...
    "import jxlDecoder from 'sqush-generated/codecs/jxl/dec/jxl_dec';",
    ...
    "import qoiEncoder from 'sqush-generated/codecs/qoi/enc/qoi_enc';",
    ...
    "import jxlEncoder from 'sqush-generated/codecs/jxl/enc/jxl_enc';",
    ...
    "import initResizeWasm, { resize as wasmResize } from 'sqush-generated/codecs/resize/pkg/squoosh_resize';",
    "import initHqxWasm, { resize as wasmHqx } from 'sqush-generated/codecs/hqx/pkg/squooshhqx';",
    ...
    '  loadSingleThread: async () => avifEncoder,',
    '});',
    ...
    '  supportsThreads: async () => false,',
    '  supportsSimd: async () => false,',
    '  loadSingleThread: async () => jxlEncoder,',
    '  async loadMultiThread() {',
    "    throw new Error('JPEG XL multithread runtime is unavailable in the SvelteKit prototype.');",
    '  },',
    '  async loadMultiThreadSimd() {',
    "    throw new Error('JPEG XL multithread SIMD runtime is unavailable in the SvelteKit prototype.');",
    '  },',
    '});',
...
function generateAvifCodecAssets() {
  return [
    '// This file is autogenerated by prototypes/sveltekit/scripts/sync-sqush-prototype.mjs',
    '// It is the prototype canonical asset manifest for AVIF codec WASM URLs.',
    '',
    ...
    '',
    'export { avifDecoderWasmUrl, avifEncoderWasmUrl };',
    '',
...
function generateServiceWorkerCachePlan() {
  return [
    ...
    "import { precacheCodecAssetUrls } from '../codec-assets/precache';",
    '',
    'export const generatedCodecCacheEntries = [',
    '  {',
    '    main: svelteKitFeaturesWorkerUrl,',
    '    deps: precacheCodecAssetUrls,',
    '  },',
...
function generateCodecAssetManifest() {
  return [
    ...
    '',
    "import { avifDecoderWasmUrl, avifEncoderWasmUrl } from './avif';",
    "import { hqxWasmUrl, resizeWasmUrl } from './resize';",
    "import { imagequantWasmUrl } from './imagequant';",
    "import { jxlDecoderWasmUrl, jxlEncoderWasmUrl } from './jxl';",
    "import { mozjpegEncoderWasmUrl } from './mozjpeg';",
    "import { oxipngWasmUrl } from './oxipng';",
    "import { qoiDecoderWasmUrl, qoiEncoderWasmUrl } from './qoi';",
    "import { rotateWasmUrl } from './rotate';",
    "import { webpDecoderWasmUrl, webpEncoderSimdWasmUrl, webpEncoderWasmUrl } from './webp';",
    '',
    ...
    "  { logicalKey: 'avif:decoder:default', codec: 'avif', role: 'decoder', variant: 'default', url: avifDecoderWasmUrl, cache: 'precache' },",
    "  { logicalKey: 'avif:encoder:single-thread', codec: 'avif', role: 'encoder', variant: 'single-thread', url: avifEncoderWasmUrl, cache: 'precache' },",
    "  { logicalKey: 'webp:decoder:default', codec: 'webp', role: 'decoder', variant: 'default', url: webpDecoderWasmUrl, cache: 'precache' },",
    "  { logicalKey: 'webp:encoder:baseline', codec: 'webp', role: 'encoder', variant: 'baseline', url: webpEncoderWasmUrl, cache: 'precache' },",
    "  { logicalKey: 'webp:encoder:simd', codec: 'webp', role: 'encoder', variant: 'simd', url: webpEncoderSimdWasmUrl, cache: 'precache' },",
    "  { logicalKey: 'qoi:decoder:default', codec: 'qoi', role: 'decoder', variant: 'default', url: qoiDecoderWasmUrl, cache: 'precache' },",
    "  { logicalKey: 'qoi:encoder:default', codec: 'qoi', role: 'encoder', variant: 'default', url: qoiEncoderWasmUrl, cache: 'precache' },",
    "  { logicalKey: 'jxl:decoder:default', codec: 'jxl', role: 'decoder', variant: 'default', url: jxlDecoderWasmUrl, cache: 'precache' },",
    "  { logicalKey: 'jxl:encoder:single-thread', codec: 'jxl', role: 'encoder', variant: 'single-thread', url: jxlEncoderWasmUrl, cache: 'precache' },",
    "  { logicalKey: 'mozjpeg:encoder:default', codec: 'mozjpeg', role: 'encoder', variant: 'default', url: mozjpegEncoderWasmUrl, cache: 'precache' },",
    "  { logicalKey: 'oxipng:encoder:single-thread', codec: 'oxipng', role: 'encoder', variant: 'single-thread', url: oxipngWasmUrl, cache: 'precache' },",
    "  { logicalKey: 'imagequant:processor:default', codec: 'imagequant', role: 'processor', variant: 'default', url: imagequantWasmUrl, cache: 'precache' },",
    "  { logicalKey: 'resize:processor:default', codec: 'resize', role: 'processor', variant: 'default', url: resizeWasmUrl, cache: 'precache' },",
    "  { logicalKey: 'hqx:processor:hqx', codec: 'hqx', role: 'processor', variant: 'hqx', url: hqxWasmUrl, cache: 'precache' },",
    "  { logicalKey: 'rotate:preprocessor:default', codec: 'rotate', role: 'preprocessor', variant: 'default', url: rotateWasmUrl, cache: 'runtime' },",
    '] satisfies readonly CodecAssetRecord[];',
    '',
    'export const precacheCodecAssetRecords = svelteKitCodecAssetRecords.filter(',
...
function generateCodecAssetPrecacheManifest() {
  return [
    ...
    "import { qoiDecoderWasmUrl, qoiEncoderWasmUrl } from './qoi';",
    "import { webpDecoderWasmUrl, webpEncoderSimdWasmUrl, webpEncoderWasmUrl } from './webp';",
    "import type { CodecAssetRecord } from './manifest';",
    '',
...
async function patchEmscriptenWrapperFallbackUrl({
  ...
  const replacements = [
    {
      pattern: `var wasmBinaryFile=new URL("${assetName}",import.meta.url).toString()`,
      replacement: `var wasmBinaryFile="${assetName}"`,
    },
    {
      pattern: `wasmBinaryFile=new URL("${assetName}",import.meta.url).toString()`,
      replacement: `wasmBinaryFile="${assetName}"`,
    },
  ];
  const matchedReplacement = replacements.find(({ pattern }) =>
  ...
  return [
    ...
    source.replace(matchedReplacement.pattern, matchedReplacement.replacement),
  ].join('\n');
}

async function patchWasmBindgenWrapperFallbackUrl({ sourcePath, assetName }) {
  const source = await readFile(sourcePath, 'utf8');
  const pattern = `input = new URL('${assetName}', import.meta.url);`;
  const replacement = `input = '${assetName}';`;
